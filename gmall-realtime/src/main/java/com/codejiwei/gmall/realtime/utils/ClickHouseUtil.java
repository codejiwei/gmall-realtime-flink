...
import com.codejiwei.gmall.realtime.bean.TransientSink;
...
public class ClickHouseUtil {
    ...
    /*
     * @Desc: //TODO 获取向ClickHouse中写入数据的SinkFunction
     * @Param: [sql]
    ...
    public static <T> SinkFunction getJdbcSink(String sql) {
        SinkFunction<T> sinkFunction = JdbcSink.<T>sink(
                //要执行的SQL语句
                sql,
                //执行写入操作，就是将当前流中的对象属性赋值给SQL的占位符 insert into visitor_stats_2021 value(?,?,?,?,?,?,?,?,?,?,?,?)
                new JdbcStatementBuilder<T>() {
                    ...
                    public void accept(PreparedStatement ps, T obj) throws SQLException {
                        ...
                        for (int i = 0; i < fields.length; i++) {
                            ...
                            //如果transientSink不为空，那么说明属性上有@TransientSink标记，那么在给?占位符赋值的时候，应该跳过当前属性
                ...
                //构建者设计模式，创建JdbcExecutionOptions对象，给batchSize属性赋值，执行执行批次大小:这里设置的是5个一个批次
